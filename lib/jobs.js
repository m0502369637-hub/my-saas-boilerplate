...
export async function pollJob(job, { force = false } = {}) {
  ...
  if (result.status === 'complete') {
    if (!result.output?.url) return failJob(fresh, 'Provider completed without a result URL');
    return completeJob(fresh, result.output);
  }
...
/**
 * Mark complete + deliver the result. Accepts either a plain video URL
 * (legacy form) or `{ url, kind: 'video' | 'image' }` — providers return the
 * latter, and delivery auto-adapts (sendVideo / sendPhoto). The conditional
 * update is the cross-invocation guard: if another poll already completed
 * this job, we touch nothing and send nothing twice.
 */
export async function completeJob(job, output) {
  const out = typeof output === 'string' ? { url: output, kind: 'video' } : output;
  const claimed = await db.update(jobs)
    .set({ status: 'complete', outputUrl: out.url, updatedAt: new Date() })
  ...
  const fresh = await getJob(job.id);
  await addEvent(job.id, 'completed', { outputUrl: out.url, kind: out.kind });

  try {
    await deliverResult(fresh, out);
  } catch (e) {
    console.warn('result delivery failed', { jobId: job.id, reason: e?.message });
    // Delivery is the product — if the user never received the result,
    // refund. Only a job we just completed may be failed this way.
    return failJob(fresh, `Result delivery failed: ${e?.message ?? 'unknown'}`, { fromComplete: true });
  }
  return getJob(job.id);
}
...
/**
 * Send the finished result, adapting to its kind:
 ...
 */
async function deliverResult(job, out) {
  const isImage = out.kind === 'image';
  const caption = isImage ? `🖼 ${job.serviceName} — enjoy!` : `🎬 ${job.serviceName} — enjoy!`;

  if (job.provider === 'comfyui' && secrets.comfyuiApiKey) {
    const bytes = await comfyui.downloadOutput(out.url);
    const file = isImage
      ? new InputFile(bytes, 'result.png', { type: 'image/png' })   // ComfyUI SaveImage default is PNG
      : new InputFile(bytes, 'video.mp4', { type: 'video/mp4' });
    if (isImage) return safe(() => api.sendPhoto({ chat_id: job.userTgId, photo: file, caption }));
    return safe(() => api.sendVideo({ chat_id: job.userTgId, video: file, caption, supports_streaming: true }));
  }

  if (isImage) {
    return safe(() => api.sendPhoto({ chat_id: job.userTgId, photo: out.url, caption }));
  }
  return safe(() => api.sendVideo({ chat_id: job.userTgId, video: out.url, caption, supports_streaming: true }));
}
