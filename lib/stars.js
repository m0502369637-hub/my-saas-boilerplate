...
// The invoice payload is an opaque id, NOT an amount — services may cost
// different amounts of Stars, and the credit always uses Telegram's trusted
// `total_amount` from the payment itself.
export const INVOICE_PAYLOAD = 'buy_stars';
 ...
 */
export async function createStarsInvoice(chatId, amount, description, payload = INVOICE_PAYLOAD) {
...
export async function handleSuccessfulPayment(message) {
  ...
  await notify(chatId, `✅ ${amount} ⭐ added to your balance. Send /help to see what I can do!`);
  return amount;
}
