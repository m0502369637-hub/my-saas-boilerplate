...
/**
 * Submit a job to the queue. NEVER awaits completion — the queue submit
 ...
 */
export async function submitRequest({ model, input, webhookUrl = null }) {
  assertKey();
...
/** Back-compat alias for the original image-to-video submitter. */
export async function submitImageToVideo(opts) {
  return submitRequest(opts);
}
...
/** Same idea for image models (flux → { images: [{ url }] }). */
...
/**
 * Normalize any fal payload into { url, kind: 'video' | 'image' } — the
 * state machine delivers whichever kind came back, so one service can
 * produce images and another videos without touching lib/jobs.js.
 */
export function extractOutput(payload) {
 ...
 *          { status: 'complete', output: { url, kind } } on success,
