...
/** Same walk for image outputs (SaveImage → { images: [{ filename, … }] }). */
export function getOutputImageUrl(history, promptId, outputClass = 'SaveImage') {
  const entry = historyEntry(history, promptId);
  if (!entry) return null;
  const outputs = entry.outputs ?? {};
  const graph = findGraph(entry);
  ...
  for (const [nodeId, out] of Object.entries(outputs)) {
    ...
    const url = fileUrl(image, baseUrl());
    if (!url) continue;
    const cls = graph?.[nodeId]?.class_type;
    if (!outputClass || cls === outputClass) return url;
    if (!fallback) fallback = url;
 ...
 *          (kind is auto-detected: 'video' for VHS/SaveVideo outputs,
 *           'image' for SaveImage outputs),
...
export async function getJobStatus(job) {
  ...
  if (videoUrl) return { status: 'complete', failed: false, output: { url: videoUrl, kind: 'video' } };
  const imageUrl = getOutputImageUrl(history, promptId);
  if (imageUrl) return { status: 'complete', failed: false, output: { url: imageUrl, kind: 'image' } };
